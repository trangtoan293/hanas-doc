...
class ColumnLineagePublisher:
    ...
    def __init__(
        ...
        Args:
            ...
            platform: Primary platform for lineage (default: iceberg)
            platform_instance: Platform instance name (e.g., 'demo')
    ...
    def _build_dataset_urn(
        ...
        include_platform_instance: bool = True,
    ) -> str:
        """
        Build a DataHub dataset URN.
        
        Args:
            database: Database name (may be empty for some platforms)
            schema: Schema name
            table: Table name
            target_platform: Platform to use (defaults to self.platform)
            include_platform_instance: Whether to include platform_instance in dataset name
                - True: demo.integration.hub_gl (for iceberg)
                - False: integration.hub_gl (for dbt)
        """
        platform = target_platform or self.platform
        parts = [p for p in [database, schema, table] if p]
        full_name = ".".join(parts)
        
        if include_platform_instance and self.platform_instance:
            full_name = f"{self.platform_instance}.{full_name}"
        
        return f"urn:li:dataset:(urn:li:dataPlatform:{platform},{full_name},{self.env})"

    def _build_iceberg_urn(self, database: str, schema: str, table: str) -> str:
        """Build URN for iceberg platform (with platform_instance prefix)."""
        return self._build_dataset_urn(
            database, schema, table,
            target_platform="iceberg",
            include_platform_instance=True,
        )

    def _build_dbt_urn(self, database: str, schema: str, table: str) -> str:
        """
        Build URN for dbt platform (WITHOUT platform_instance prefix).
        
        Based on observed URN patterns:
        - Iceberg: urn:li:dataset:(urn:li:dataPlatform:iceberg,demo.integration.hub_gl,PROD)
        - dbt: urn:li:dataset:(urn:li:dataPlatform:dbt,integration.hub_gl,PROD)
        """
        return self._build_dataset_urn(
            database, schema, table,
            target_platform="dbt",
            include_platform_instance=False,  # dbt URN does NOT include platform_instance
        )
    ...
    def _normalize_upstream_urn(self, urn: str) -> str:
        """
        Normalize upstream URN to ensure it has the correct platform_instance prefix.
        ...
        pattern = rf"urn:li:dataset:\(urn:li:dataPlatform:{self.platform},([^,]+),([^)]+)\)"
        ...
        # If dataset_name already starts with platform_instance, return as-is
        if dataset_name.startswith(f"{self.platform_instance}."):
            return urn
        
        # Add platform_instance prefix
        normalized_name = f"{self.platform_instance}.{dataset_name}"
        return f"urn:li:dataset:(urn:li:dataPlatform:{self.platform},{normalized_name},{env})"
    ...
    def _convert_iceberg_urn_to_dbt_urn(self, iceberg_urn: str) -> str:
        ...
        2. Removing platform_instance prefix from dataset name
        
        Example:
        - Input: urn:li:dataset:(urn:li:dataPlatform:iceberg,demo.integration.hub_gl,PROD)
        - Output: urn:li:dataset:(urn:li:dataPlatform:dbt,integration.hub_gl,PROD)
        """
        if not self.platform_instance:
            # Just change platform name
        ...
        # Remove platform_instance prefix if present
        prefix = f"{self.platform_instance}."
        if dataset_name.startswith(prefix):
            dataset_name = dataset_name[len(prefix):]
        
        return f"urn:li:dataset:(urn:li:dataPlatform:dbt,{dataset_name},{env})"
        ...
        for cl in column_lineage:
            try:
                ...
                for up in upstreams:
                    ...
                    if f"dataPlatform:{self.platform}" not in up_urn_str:
                        logger.debug(
                            "Skipping non-%s upstream: %s", self.platform, up_urn_str
                        )
                        continue
    ...
    def publish_from_dbt_artifacts(
        ...
        for unique_id, node in nodes.items():
            ...
            if not sql.strip():
                skipped += 1
                logger.debug("No compiled SQL for %s", name)
                continue

            # Build iceberg URN (primary platform)
            iceberg_downstream_urn = self._build_iceberg_urn(database, schema, name)
            ...
            if not fine_grained_lineages:
                skipped += 1
                logger.debug("No column lineage found for %s", name)
                continue
...
def publish_column_lineage(
    ...
    run_results_path: str,
    platform: str = "iceberg",
    platform_instance: Optional[str] = "demo",
    env: str = "PROD",
    clear_existing: bool = True,
    emit_to_dbt_platform: bool = True,
) -> Dict[str, Any]:
    ...
    Args:
        ...
        platform: DataHub platform (default: iceberg)
        platform_instance: Platform instance (default: demo)
        env: Environment (default: PROD)
        clear_existing: If True (default), delete existing lineage before
...
def main():
    ...
    parser.add_argument(
        "--platform", default="iceberg", help="DataHub platform (default: iceberg)"
    )
    parser.add_argument(
        "--platform-instance", default="demo", help="Platform instance (default: demo)"
    )
    parser.add_argument(
