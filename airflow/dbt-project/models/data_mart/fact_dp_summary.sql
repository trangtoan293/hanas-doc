...
{#
    Fact Table: Deposit Summary (Snapshot by COB_DATE)
    
    Incremental strategy: insert_overwrite
    - Partition by COB_DATE, overwrite entire partition on each run
    
    MTD Calculation:
    - CTE_DP_ALL: All historical data (no filter) - used for MTD sum
    - CTE_DP_CURR: Filtered by ref_dates in incremental mode
    - CTE_DP_MTD: Joins CURR with ALL to get correct MTD
#}
...
-- All historical data (unfiltered) - needed for MTD calculation and historical lookups
CTE_DP_ALL AS
(
select TT.COB_DATE,
       ...
       DP.ACCOUNT_TYPE,
       DP.LCY_CURR_BALANCE 
from {{ ref('fact_dp_detail') }} DP
JOIN {{ ref('dim_time') }} TT
                        ON DP.COB_DATE = TT.COB_DATE
)

-- Current data filtered for incremental processing
, CTE_DP_CURR AS
(
SELECT * FROM CTE_DP_ALL
{% if is_incremental() %}
WHERE COB_DATE IN (SELECT cob_date FROM ref_dates)
{% endif %}
)

-- Day Ago: lookup from ALL historical data
, CTE_DP_DAGO AS
(
select TT.COB_DATE,
       CAST(TT.ELM_DATE AS DATE) ELM_DATE,
...
{% if is_incremental() %}
WHERE TT.COB_DATE IN (SELECT cob_date FROM ref_dates)
{% endif %}
)

-- End of Last Month: lookup from ALL historical data
, CTE_DP_ELM AS
(select TT.COB_DATE,
       CAST(TT.ELM_DATE AS DATE) ELM_DATE,
       CAST(TT.ELY_DATE AS DATE) ELY_DATE,
...
-- End of Last Year: lookup from ALL historical data
, CTE_DP_ELY AS
(select TT.COB_DATE,
       CAST(TT.ELM_DATE AS DATE) ELM_DATE,
       CAST(TT.ELY_DATE AS DATE) ELY_DATE,
...
-- MTD: Join CURR (today's records) with ALL (historical data for sum)
,CTE_DP_MTD AS
(
select SS.COB_DATE,
       SS.ELM_DATE,
...
FROM
    ...
    select DP.COB_DATE,
       ...
       TT.LCY_CURR_BALANCE
    from CTE_DP_CURR DP
    JOIN CTE_DP_ALL TT  -- Use ALL for historical MTD data
               ON TT.COB_DATE BETWEEN DP.BOM_DATE AND DP.COB_DATE  
              AND DP.D_BRANCH_ID = TT.D_BRANCH_ID
              AND DP.CUSTOMER_TYPE = TT.CUSTOMER_TYPE 
              AND DP.ACCOUNT_TYPE = TT.ACCOUNT_TYPE   
    ) SS
GROUP BY
       SS.COB_DATE,
       SS.ELM_DATE,
