...
{#
    Fact Table: Loan Summary (Snapshot by COB_DATE)
    
    Incremental strategy: insert_overwrite
    - Partition by COB_DATE, overwrite entire partition on each run
    
    MTD Calculation:
    - CTE_LN_ALL: All historical data (no filter) - used for MTD sum
    - CTE_LN_CURR: Filtered by ref_dates in incremental mode
    - CTE_LN_MTD: Joins CURR with ALL to get correct MTD
#}
...
FROM
    ...
    select CL.COB_DATE,
       ...
       TT.LCY_OUTSTND
    from CTE_LN_CURR CL
    JOIN CTE_LN_ALL TT  -- Use ALL for historical MTD data
